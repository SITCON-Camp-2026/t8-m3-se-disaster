...
- 不做後端、資料庫、localStorage、外部 API 或地圖
- 不使用 runtime LLM
- 不做真實救災判斷
- 不修改 `CommonRecord`
- 不把未整理資料移進 `src/fixtures/shared/` 假裝乾淨
- 不把 `needs_review` 或 `unverified` 顯示成 verified / confirmed
- 不把 M-001 到 M-012 的分析 hardcode 成標準答案表
...
- [ ] 至少 2 個 agent 判斷被人類質疑或修正，並能在 UI 或 observation 看見
